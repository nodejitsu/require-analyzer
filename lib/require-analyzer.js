...
analyzer.dir = function (options, callback) {  
  ...
  fs.readdir(options.target, function (err, files) {
    ...
    //
    if ((options.file && files.indexOf(options.file)) !== -1 || files.indexOf('package.json') !== -1) {
    ...
    try {
      ...
      function processOptions(options) {
        ...
        analyzer.file(options, function (err, deps) {
          _deps.concat(deps.filter(function (d) { return _deps.indexOf(d) === -1 && d !== pkg.name }));
          dequeue(err);
        });
      }
      var scripts = options.hasOwnProperty('scripts') ? [].concat(options.scripts) : ["test","prestart"];
      if(scripts) {
        scripts.forEach(function analyzeScript(script) {
          if(!script) return;
          var options = analyzer.clone(options);
          options.target = require.resolve(path.join(options.target, path.normalize(script)));
          processOptions(options);
        });
      }
      
      var options = analyzer.clone(options);
      options.target = require.resolve(path.join(options.target, path.normalize(pkg.main || '/')));
      processOptions(options);
    }
    catch (ex) {
       return callback(ex);
