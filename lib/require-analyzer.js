    ...
    try {
      ...
      function processOptions(options) {
        ...
        analyzer.file(options, function (err, deps) {
          _deps = _deps.concat(deps.filter(function (d) { return _deps.indexOf(d) === -1 && d !== pkg.name }));
          dequeue(err);
        });
      }
      var scripts = options.hasOwnProperty('scripts') ? [].concat(options.scripts).map(function(item){
  ...
  fs.stat(target, function (err, stats) {
    ...
    if (stats.isDirectory()) {
      return fs.readdir(target, function (err, files) {
        ...
        if (files.indexOf('node_modules') !== -1 || files.indexOf('package.json')) {
