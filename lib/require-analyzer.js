...
analyzer.npmAnalyze = function (deps, options, callback) {
  ...
  async.forEach([options.npm.log, options.npm.out], unlinkFile, function () {
    ...
    npm.load(npmOptions, function (err) {
      ...
      //
      // Analyze each dependency by searching for it on npm and finding the 
      // currently active version. Then see if it depends on any other dependencies 
      // that are in the list so those dependencies may be removed.
      //
